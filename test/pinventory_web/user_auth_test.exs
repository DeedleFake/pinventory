defmodule PinventoryWeb.UserAuthTest do
  ...
  @remember_me_cookie "_pinventory_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PinventoryWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PinventoryWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_sudo_mode" do
    test "allows users that authenticated within the sudo window", %{conn: conn, user: user} do
      user_token = Accounts.generate_user_session_token(user)
      session = conn |> put_session(:user_token, user_token) |> get_session()
    ...
    test "redirects when authentication is too old", %{conn: conn, user: user} do
      twenty_one_minutes_ago = DateTime.utc_now(:second) |> DateTime.add(-21, :minute)
      user = %{user | authenticated_at: twenty_one_minutes_ago}
      user_token = Accounts.generate_user_session_token(user)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        PinventoryWeb.Endpoint.subscribe("user_sessions:#{Base.url_encode64(token)}")
      end
