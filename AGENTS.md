...
- Run `mix dialyzer` for static type analysis when you change public APIs, schemas, or complex control flow. CI also runs Dialyzer. The first run builds PLTs under `priv/plts/` and can take several minutes; later runs are faster.
...
- **Always** be mindful of where to place routes. `phx.gen.auth` creates multiple router plugs and `live_session` scopes:
  ...
  - A plug `redirect_if_user_is_authenticated` that redirects to a default path in case the user is authenticated - useful for a registration page that should only be shown to unauthenticated user
