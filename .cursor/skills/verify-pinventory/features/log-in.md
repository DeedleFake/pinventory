...
- `login-form` shows Email, Password, and Keep me logged in when users exist.
...
- **Form.** Confirm fields. Run `verify-pinventory browser exists '#user_email'` and `verify-pinventory browser exists '#user_password'` and `verify-pinventory browser exists '#user_remember_me'`.
...
- `Keep me logged in` is checked by default (`#user_remember_me`).
